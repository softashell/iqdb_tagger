    ...
    def index(self) -> Any:
        ...
        if form.file.data:
            ...
            with NamedTemporaryFile(delete=False) as temp, NamedTemporaryFile(
                ...
                place = [
                    x[1] for x in form.place.choices if x[0] == int(form.place.data)
                ][0]
                url, im_place = iqdb_url_dict[place]
                query = (
                    posted_img.imagematchrelationship_set.select()
                    .join(ImageMatch)
                ...
                if not query.exists():
                    try:
                        posted_img_path = (
                            temp.name if not form.resize.data else thumb_temp.name
                        )
            ...
            return redirect(
                url_for("matchview.match_sha256", checksum=posted_img.checksum)
            )
    ...
    def match_detail(self, pair_id: str) -> Any:
        ...
        if urlparse(match_result.link).netloc in filtered_hosts:
            current_app.logger.debug(
                "URL in filtered hosts, no tag fetched, url:{}".format(
                    match_result.link
                )
            )
